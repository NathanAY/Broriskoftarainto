...
- **Types & examples**: periodic (e.g., `RegenModifier` with a timer), reactive/spawn (e.g., `ChainModifier`, `SpreadModifier`, `HomingOnHit`, `BombModifier`), utility (e.g., shields, reflect, knockback), and life/health effects (e.g., `LifeLeachModifier`, `StatOnKillModifier` — configurable `target_stat`/`add_amount`, triggers on `on_kill` which the damage pipeline emits when a target dies).
