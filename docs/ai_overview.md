...
- **How they work**: Effects subscribe to the local `EventManager` events (e.g., `on_attack`, `on_hit`, `on_item_added`, `on_item_removed`, `on_stat_changes`) to react to gameplay. They commonly access the holder's `Stats`, `Health`, or spawn `Projectile` scenes. Many effects support stacking via `add_stack`/`remove_stack`/`set_stack_active`.
- **Types & examples**: periodic (e.g., `RegenModifier` with a timer), reactive/spawn (e.g., `ChainModifier`, `SpreadModifier`, `HomingOnHit`, `BombModifier`), utility (e.g., shields, reflect, knockback), and life/health effects (e.g., `LifeLeachModifier`, `LifeOnKillModifier`).
